...
from ..transforms import transforms as builtin_transforms
...
def get_transforms(resize: Tuple = None, resized_crop: Tuple = None,
                   ...
                   grayscale: bool = False, color: bool = False, rotate: float = None,
                   hflip: bool = False, vflip: bool = False,
                   normalize: bool = False, norm_mean: Tuple = None, norm_std: Tuple = None):
    tlist = []
    if color and grayscale: raise ValueError("color and grayscale cannot be True at the same time!")
    if resize: tlist.append(transforms.Resize(resize))
    if color: tlist.append(builtin_transforms.ToColor())
...
def get_pair_transforms(resize: Tuple = None, resized_crop: Tuple = None,
    ...
    if color: tlist.append(pair_transforms.PairToColor())
...
def imagenet_train_transform(image_size: Tuple = (224, 224), resized_crop: Tuple = (224, 224),
                             normalize=True, grayscale=False,
                             hflip=False, vlip=False, rotate=None, ):
    mean, std = get_imagenet_mean_std()
    tmft = get_transforms(resize=image_size, resized_crop=resized_crop, grayscale=grayscale,
